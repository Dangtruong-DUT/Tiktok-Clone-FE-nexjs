...
import { timeToMMSSCS } from '@/utils/formatting/format-time.util'
...
export default function VideoPreview({ videoSrc, content, className, seekTo }: VideoPreviewProps) {
    ...
    return (
        <div
            className={cn(
                'w-[264px] h-[571px] bg-transparent relative border-4 rounded-[25px] dark:border-gray-600 border-black group',
                className
            )}
        >
            ...
            <div className='absolute top-0 left-0 inset-0 z-2'>
                <div className='relative h-[511px] w-full'>
                    <video
                        ref={videoRef}
                        className={cn('w-full h-[511px] object-contain rounded-t-[20px] bg-black')}
                        muted={isMuted}
                        autoPlay
                        loop
                        preload='metadata'
