...
export const PostApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createComment: builder.mutation<
      { message: string },
      CreateCommentsReqBodyType & { post_uuid: string }
    >({
      query: (payload) => ({
        url: `/posts`,
        method: "POST",
    ...
    getListPost: builder.infiniteQuery<
      GetListPostRes,
      "friend" | "foryou",
      number
    >({
    ...
    getRelatedPosts: builder.infiniteQuery<GetListPostRes, string, number>({
      query: ({ pageParam }) =>
        `/posts?page=${pageParam}&per_page=10&type=${PosterType.POST}`,
      providesTags: (result, error, arg) => {
        if (result) {
          const final = [
            ...result.pages.flatMap((page) => {
    ...
    >({
      query: ({ userId, page }) =>
        `/users/${userId}/posts?page=${page}&per_page=10&type=${PosterType.POST}`,
      providesTags: (result) => {
        if (result) {
          const final = [
            ...result.data.map(({ id }) => ({
    ...
    getBookmarkedPostsOfUser: builder.infiniteQuery<
      GetListPostRes,
      string,
      number
    >({
    ...
    updatePost: builder.mutation<
      { message: string },
      { post_uuid: string; body: UpdatePostReqBodyType }
    >({
