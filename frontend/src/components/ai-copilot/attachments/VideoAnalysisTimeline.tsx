...
export function VideoAnalysisTimeline({ videoRef, duration, onSeek }: VideoAnalysisTimelineProps) {
    ...
    const warning: 'too_short' | 'too_long' | null =
        selDuration < TIMELINE.MIN_DURATION ? 'too_short' : selDuration > TIMELINE.MAX_DURATION ? 'too_long' : null
    ...
    const pct = (sec: number) => (duration > 0 ? Math.max(0, Math.min(100, (sec / duration) * 100)) : 0)
        ...
        (zone: DragZone) => (e: React.PointerEvent) => {
            ...
            const onMove = (ev: PointerEvent) => {
                ...
                if (zone === 'start') {
                    ...
                    ne = Math.min(
                        state.totalDuration,
                        Math.max(state.startSel.start + TIMELINE.MIN_DURATION, state.startSel.end + dSec)
                    )
    ...
    return (
        <div className='space-y-1.5'>
            ...
            >
                ...
                {/* Playhead */}
                <div
                    className='absolute inset-y-0 w-px bg-white/90 shadow pointer-events-none z-20'
