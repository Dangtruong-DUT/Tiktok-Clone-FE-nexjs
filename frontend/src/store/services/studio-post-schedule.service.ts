...
export const StudioPostScheduleApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        listScheduledPosts: builder.query<ApiSuccessResponseWithMeta<ScheduledPostType[]>, { page?: number; per_page?: number }>({
            query: (params) => ({ url: BACKEND_API_ENDPOINT.STUDIO_POSTS.SCHEDULED, params }),
            providesTags: ['ScheduledPost'],
        }),

        schedulePost: builder.mutation<ApiSuccessResponseWithData<ScheduledPostType>, { postUuid: string } & SchedulePostBody>({
            query: ({ postUuid, ...body }) => ({
        ...
        reschedulePost: builder.mutation<ApiSuccessResponseWithData<ScheduledPostType>, { schedUuid: string } & SchedulePostBody>({
